...
class TSVM(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        # finding w_A, b_A
        K = matrix(GB.dot(np.linalg.inv(HA.T.dot(HA) + 1*I)).dot(GB.T))
        q = matrix((-e_B))
        ...
        self.u = -np.linalg.inv(((HA.T).dot(HA) + 1*I)).dot(GB.T).dot(alpha)
        self.b_A = self.u[-1]
        self.w_A = self.u[:-1]     
    
        # finding w_B, b_B
        K = matrix(HA.dot(np.linalg.inv(GB.T.dot(GB) + 1*I)).dot(HA.T))
        q = matrix(-e_A)
        ...
        sol = solvers.qp(K, q, G, h)
        beta = np.array(sol['x'])
        self.v = np.linalg.inv(GB.T.dot(GB) + 1*I).dot(HA.T).dot(beta)
        self.b_B = self.v[-1]
        self.w_B = self.v[:-1]
