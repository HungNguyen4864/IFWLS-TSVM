...
def Pin_LGBTSVM(Data,TestX,d1, d2, d3, d4,tau):
    C1 = Data[Data[:, -1] == 1, :-1]
    C2 = Data[Data[:, -1] != 1, :-1]
    A = C1[:,:-1]
    B=C2[:,:-1]
    ...
    p = A.shape[0]
    q = B.shape[0]
    lb1 = np.concatenate((np.zeros(p), tau*d1 * np.ones(q)))
    ub1 = np.concatenate((np.zeros(p), d1 * np.ones(q)))
    f1 = -d3 * np.concatenate((np.zeros(p), np.ones(q)+R2))

    #Q1 = np.block([[np.dot(A , A.T) + d3 * np.eye(p), np.dot(A, B.T)], [np.dot(B, A.T), np.dot(B, B.T)]]) + np.ones((p + q, p + q))
    ...
    lb2 = np.concatenate((np.zeros(q), tau*d2 * np.ones(p)))
    ub2 = np.concatenate((np.zeros(q), d2 * np.ones(p)))
    f2 = -d4 * np.concatenate((np.zeros(q), np.ones(p)+R1))

    #Q2 = np.block([[np.dot(B, B.T) + d4 * np.eye(q), np.dot(B, A.T)], [np.dot(A, B.T), np.dot(A, A.T)]]) + np.ones((p + q, p + q))
